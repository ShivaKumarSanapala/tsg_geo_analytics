...
do
    ...
    output_file="data/${year}_counties_demography.csv"
    ./fetch_data_from_acs.sh "$api_url" "$query_string" "$variable_mapping" > "$output_file"
    echo "Data for ${year} counties saved to ${output_file}"

    query_string="get=NAME,GEO_ID,B01003_001E,B01001_026E,B25064_001E,B19013_001E,B15002_015E,B15002_032E&for=state:*"
    output_file="data/${year}_states_demography.csv"
    ./fetch_data_from_acs.sh "$api_url" "$query_string" "$variable_mapping" > "$output_file"
    echo "Data for ${year} states saved to ${output_file}"
done
